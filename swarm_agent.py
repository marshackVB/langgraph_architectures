...
class SwarmAgent(ChatAgent):
    ...
    def _generate_deduplicated_chunks(self, messages):
        """
        LangGraph Swarms return duplicate messages within as the conversation is passed
        across agents. This filters through the messages ids generated by LangGraph's
        stream method and only returns distinct messages
        """
            ...
            for agent, data in event.items():
                if "messages" in data:
                    for msg in data["messages"]:
                        ...
                        dedup_key = msg_id if msg_id else hash((msg.__class__.__name__, msg.content))
                        if dedup_key in seen_message_ids:
                            continue
                        seen_message_ids.add(dedup_key)

                        converted_message = convert_to_openai_messages(msg)
                        openai_format_with_id = converted_message | {"id": str(uuid.uuid4())}
                        yield openai_format_with_id
    ...
    def predict(self,
        ...
        messages = {"messages": self._convert_messages_to_dict(messages)}
        
        response = []
        for openai_format_with_id in self._generate_deduplicated_chunks(messages):
            response.extend([ChatAgentMessage(**openai_format_with_id)])
